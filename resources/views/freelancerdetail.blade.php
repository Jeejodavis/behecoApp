    ...
    <section class="profile">
        <div class="cover-bg">
            @if (!empty($detail->cover_photo))
                <img src="{{ asset('images/freelancer/' . $detail->id . '/' . $detail->cover_photo) }}" alt="cover-img">
            @else
        ...
        <div class="container">
                ...
                <div class="col-xl-5">
                    <div class="profile-pic">
                        @if (!empty($detail->profile_photo))
                            <img src="{{ asset('images/freelancer/' . $detail->id . '/' . $detail->profile_photo) }}"
                            alt="profile-image">
                        @else
                    ...
                    <div class="general-info">
                        <div class="title">
                            <h2>{{ $detail->freelancer_name }}<i class="fas fa-check-circle"></i></h2>
                ...
                <div class="demo-gallery">
                    <ul id="lightgallery" class="row">
                        @foreach ($photos as $key => $photo)
                            <li class="col-xl-2 col-md-4 col-sm-4 col-6" data-src="{{ asset('images/freelancer/' . $photo->freelancer_id . '/photos/' . $photo->photo) }}">
                                <a href="">
                                    <img class="img-responsive"
                                        src="{{ asset('images/freelancer/' . $photo->freelancer_id . '/photos/' . $photo->photo) }}">
                        ...
                        <div class="slider">
                            <div class="imgs">
                                @foreach ($offers as $ofKey => $offer)
                                    <img src="{{ asset('images/freelancer/' . $offer->freelancer_id . '/offer/' . $offer->offer_image) }}" class="img" data-bs-toggle="modal"
                            ...
                            <div class="col-xl-12">
                                <div class="rating-form">
                                    <form name="review_form" id="review_form" method="post" enctype="multipart/form-data" action="addReview">
                                        <input type="hidden" name="freelancer_id" value="{{ $detail->id }}">
                                        <h4>Rate Me</h4>
                                        <div class="mb-3">
                                        ...
                                        <div class="mb-3">
                                            @auth
                                                <button type="button" id="free_submit_review" class="btn btn-general">Post Review</button>
                                            @endauth
                                            @guest
            ...
            <div class="questions">
                ...
                <div class="question-form">
                    ...
                    <div class="mb-3">
                        @auth
                            <button type="button" id="free_add_question" class="btn btn-general">Ask Question</button>
                        @endauth
                        @guest
                                    ...
                                    <div class="main-content">
                                        ...
                                        <div class="auther-info">
                                            ...
                                            @if (empty($question['answer']) && isset(auth()->user()->id) && $detail->user_id == auth()->user()->id)
                                                <a class="a-animate reply-qn freelancerQnReply" quesId="{{ $question['ques_id'] }}" >Reply</a>
                                            @endif
                                        </div>
    ...
    <section class="similar-result slider-wrapper">
        <div class="container">
            <div class="hs__header">
                <h2 class="hs__headline common-h2">Similar <span>Freelancers</span>
                </h2>
                <div class="navigation-btns hs__arrows">
                    @if (count($similarfreelancer) > 3)
            ...
            <div class="hs__wrapper">
                <ul class="hs" style="float: left;">
                    @foreach ($similarfreelancer as $sKey => $similar)
                        <li class="hs__item">
                            <div class="content-box">
                                <div class="image-wrapper">
                                    @if (!empty($similar['profile_photo']))
                                        <img src="{{ asset('images/freelancer/' . $similar['id'] . '/' . $similar['profile_photo']) }}" alt="image">
                                    @else
                                ...
                                <div class="txt-section">
                                    <div class="flex-container top-section">
                                        <h5>{{ $similar['freelancer_name'] }}</h5>
                                        <div class="right-align">
                                            @auth
                                                <a type="button" class="freelancerSavedItem" freelancerId="{{ $similar['id'] }}" saveType="add"><i class="fas fa-heart"></i></a>
                                            @endauth
                                            @guest
                    ...
                    <div class="flex-h-cntr pagination-wrapper">
                        @if ($rating->review_count > 10)
                            ...
                            ?>
                                ...
                                <a class="prev page-numbers disabled" id="freeReviewPrev" ><i class="fas fa-angle-double-left"></i>
                                        <span>Prev</span></a>
                                @for ($i=1; $i<=$pages; $i++)
                                    <a class="page-numbers free_review_pages {{ $i==1 ? 'current disabled' : '' }}" id="reviewPage_{{ $i }}" >{{ $i }}</a>
                                @endfor
                                <a class="next page-numbers" id="freeReviewNext"><span>Next</span> <i class="fas fa-angle-double-right"></i></a>
                            </div>
                        @endif
                    </div>
                </div>
                            ...
                            <div class="flex-h-cntr pagination-wrapper">
                                @if ($photoCount > 18)
                                    ...
                                    ?>
                                        ...
                                        <a class="prev page-numbers disabled" id="freePhotoPrev" ><i class="fas fa-angle-double-left"></i>
                                                <span>Prev</span></a>
                                        @for ($i=1; $i<=$photoPages; $i++)
                                            <a class="page-numbers free_photo_pages {{ $i==1 ? 'current disabled' : '' }}" id="photoPage_{{ $i }}" >{{ $i }}</a>
                                        @endfor
                                        <a class="next page-numbers" id="freePhotoNext"><span>Next</span> <i class="fas fa-angle-double-right"></i></a>
                                    </div>
                                @endif
                            </div>
                        </div>
                    ...
                    <div class="flex-h-cntr pagination-wrapper">
                        @if ($questionCount > 10)
                            ...
                            ?>
                                ...
                                <a class="prev page-numbers disabled" id="freeQuesPrev" ><i class="fas fa-angle-double-left"></i>
                                        <span>Prev</span></a>
                                @for ($i=1; $i<=$quesPages; $i++)
                                    <a class="page-numbers free_ques_pages {{ $i==1 ? 'current disabled' : '' }}" id="quesPage_{{ $i }}" >{{ $i }}</a>
                                @endfor
                                <a class="next page-numbers" id="freeQuesNext"><span>Next</span> <i class="fas fa-angle-double-right"></i></a>
                            </div>
                        @endif
                    </div>
                </div>
    ...
    @foreach ($offers as $ofKey => $offer)
        <div class="modal fade" id="offerModal_{{ $offer->id }}" tabindex="-1" aria-labelledby="offerModalLabel" aria-hidden="true">
            <div class="modal-dialog offer-modal">
                <div class="modal-content">
                    <div class="modal-body">
                        <img src="{{ asset('images/freelancer/' . $offer->freelancer_id . '/offer/' . $offer->offer_image) }}">
                    ...
                    <div class="row">
                        <div class="col-xl-12">
                            <div class="btn-wrapper">
                                ...
                                <button type="button" class="btn btn-submit-primary" id="freelancerSubmitReply">Reply</button>
                            </div>
                        </div>
                    </div>
                </div>
