...
$('#basic_next').click(function() {
	...
	var freelancerName = $.trim($('#freelancer_name').val());
	...
	var valResult = validateBasicDetails(freelancerName, category, subcategory, street, area, city, pincode);
	if (valResult == 1) {
		$('html, body').animate({scrollTop:$('#collapseOne').offset().top}, 'slow');
		return false;
	}
	$.ajax({
        url: base_url+'/addFreelancerBasic',
        type: 'POST',
        data: {
        ...
        success: function (data) {
            ...
            {
            	$('#freelancer_id').val(data.id);
            	$('#freelancerId').val(data.id);
...
function validateBasicDetails(freelancerName, category, subcategory, street, area, city, pincode)
{
	var valFail = 0;

	if (freelancerName == '') {
		$('#name_error').removeClass('d-none');
		$('#freelancer_name').addClass('border-danger');
		valFail = 1;
	} else {
		$('#name_error').addClass('d-none');
		$('#freelancer_name').removeClass('border-danger');
	}
...
$('#addPhotos').click(function() {
    ...
    $.ajax({
        url: base_url+'/addFreelancerPhotos',
        type: 'POST',
        data: formData,
        cache: false,
        contentType: false,
...
// 	$.ajax({
//         url: base_url+'/addFreelancerLocation',
//         type: 'POST',
//         data: {
//             'freelancer_id': freelancer_id,
//             'lattitude': lattitude,
...
$('#keywordSubmit').click(function() {
    ...
    $.ajax({
        url: base_url+'/addFreelancerKeywords',
        type: 'POST',
        data: {
            'freelancer_id': freelancer_id,
            'keywords': keywords,
